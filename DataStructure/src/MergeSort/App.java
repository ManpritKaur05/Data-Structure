...
public class App {
	...
	public static void mergeSort(int inputArray[], int start, int end) {
		...
		merge(inputArray, start, mid, end);// merge sorted results into the
											// input array
	}
	...
	public static void merge(int inputArray[], int start, int mid, int end) {
		...
		int rightslot = mid + 1;// index counter for the right side of the
								// array.
		int k = 0;
		...
		// When it gets to here that means the above loop has completed. so
		// either the right side or the leftSide of the sub-array has been sorted.

		if (leftslot <= mid)// right side has been sorted.
		{
			while (leftslot <= mid)// Now we can just copy over the right side
									// elements.
			{
				tempArray[k] = inputArray[leftslot];
				leftslot++;
		...
		} else if (rightslot <= end)// left side has been sorted right is still
									// remaining.
		{
			while (rightslot <= end) {
				tempArray[k] = inputArray[rightslot];
